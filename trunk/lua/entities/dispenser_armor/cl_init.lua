include('shared.lua')
language.Add("dispenser_armor", "Armor Dispenser")
...
function ENT:DoNormalDraw( bDontDrawModel )
	...
	if ( LocalPlayer():GetEyeTrace().Entity == self.Entity and EyePos():Distance( self.Entity:GetPos() ) < rd_overlay_dist and mode != 0) then
		...
		local titanname = AM.GetResourceRefinedName("Titanium")
		if not mode or mode != 2 then
			...
			OverlayText = OverlayText .. titanname..": "..tostring(RD.GetResourceAmount(self,titanname)).."/"..tostring(RD.GetNetworkCapacity(self,titanname)).."\n"
			OverlayText = OverlayText .. "\n"
			OverlayText = OverlayText .. "Charges: "..tostring(self:GetNWInt("mining_disparm_charges")).."/"..tostring(self:GetNWInt("mining_disparm_maxcharges")).."\n"
			...
			cam.Start3D2D(pos,angle,0.03)
				...
				surface.SetTextColor(155,155,255,255)
				surface.SetTextPos(textStartPos+15,TempY)
				surface.DrawText(titanname..": "..tostring(RD.GetResourceAmount(self,titanname)).."/"..tostring(RD.GetNetworkCapacity(self,titanname)))
